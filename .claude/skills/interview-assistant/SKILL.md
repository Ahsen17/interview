...
description: Build interview preparation from this resume repository. Use when analyzing personal resume/work/project experience, updating the candidate profile, analyzing a target job, using the local technical question bank, and local or web interview experiences to create career-positioning analysis, gap analysis, study plans, mock interview questions, answer scripts, project storytelling, resume improvement suggestions, or technical interview coaching for a target technical role.
---
...
4. Generate targeted resume content when requested.
   ...
   - Generate directly usable delivery resumes under `个人简历/` using the filename format `职位-公司-简历.md`.
...
5. Normalize and use interview experience material.
   - For company-and-role-specific notes, expect filenames such as `网络面经/<目标公司>-<目标岗位>.md`.
   - For general role notes without a company target, expect filenames such as `网络面经/<目标岗位>.md`.
